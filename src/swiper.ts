...
export class Swiper extends MyDisplay {
  ...
  constructor(opt: DisplayConstructor) {
    ...
    // this._engine.gravity.x = 0.15;
    this._engine.gravity.y = 0.35;
    ...
    this._items.forEach((_item, i) => {
      const bTop = Bodies.rectangle(itemSize * i, 0, itemSize * kake, itemSize * kake, {isStatic:true, render:{visible: Conf.FLG_SHOW_MATTERJS}});
    ...
    for (let i = 0; i < num; i++) {
      ...
      Tween.set(downEl, {
        ...
        height: 1,
        scale: 1.1,
        backgroundColor: new Color(0xffffff).getStyle(),
      })
